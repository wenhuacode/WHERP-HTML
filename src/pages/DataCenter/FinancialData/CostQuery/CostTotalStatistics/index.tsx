...
import {CostTotalStatisticsQueryAPI} from "@/services/ant-design-pro/data-center/sale/sale";
...
const CostTotalStatistics = () => {
  ...
  const NewJumpPath = async (value: any) => {
    ...
    await history.push('/data_center/financial_data/cost_query/cost_total_statistics_detail', tmp)
    return true
  }
    ...
    return (
      <Table.Summary fixed>
        <Table.Summary.Row>
          ...
          <Table.Summary.Cell index={13}>{data.this_month_total_amount}</Table.Summary.Cell>
          <Table.Summary.Cell index={15}>{data.actual_amount_total.toFixed(2)}</Table.Summary.Cell>
        </Table.Summary.Row>
      </Table.Summary>
    );
  };
  ...
  return (
    <>
      <DataDetail
        ...
        onSummary={onSummary}
        requests={CostTotalStatisticsQueryAPI}
        query={state}
        actionRef={actionRef}
        formRef={formRef}
        title={"费用合计统计"}
...
export default CostTotalStatistics;
