...
import {
  createCustomerClassify, deleteCustomerClassify,
  getCustomerClassify,
  updateCustomerClassify
} from "@/services/ant-design-pro/customer_classify";
...
const CustomerClassifyTree: React.FC = () => {
  ...
  const fetchCustomerClassify = async () => {
    ...
    const menu2Node: (m: EmployeeClassData) => DataNode = (m) => {
      return {
        ...m,
        key: m.classify_no,
        title: m.classify_no + '——' + m.classify_name,
  ...
  const handleSaveCustomerClassify: (subject: EmployeeClassData) => Promise<boolean> = async (subject) => {
    ...
    const resp = subject.id ? await updateCustomerClassify(subject) : await createCustomerClassify(subject);
    hide();
  ...
  const onDeleteClassify: (item) => Promise<boolean> =  async (item) => {
    try {
      ...
      const resp = await deleteCustomerClassify(item.id);
      hide();
...
export default CustomerClassifyTree
