...
import {
  createEmployeeClassify,
  deleteEmployeeClassify,
  getEmployeeClassify,
  updateEmployeeClassify
...
const EmployeeClassifyTree: React.FC = () => {
  ...
  const handleSaveCustomerClassify: (subject: EmployeeClassData) => Promise<boolean> = async (subject) => {
    ...
    const resp = subject.id ? await updateEmployeeClassify(subject) : await createEmployeeClassify(subject);
    hide();
  ...
  const onDeleteClassify: (item) => Promise<boolean> =  async (item) => {
    try {
      ...
      const resp = await deleteEmployeeClassify(item.id);
      hide();

      if (resp && resp.success) {
        message.success('分类删除成功!');
        await fetchEmployeeClassify().then().catch();
        return true;
      }else {
...
export default EmployeeClassifyTree
